...
VIDEO_FILE="tests/videos/cam1.mp4"

echo "🎬 Starting Camera 0 stream with FFmpeg to MediaMTX"
echo "   Source: $VIDEO_FILE"
echo "   Publishing to: rtmp://${SERVER_IP}:1935/camera0"
...
if [ ! -f "$VIDEO_FILE" ]; then
    ...
    while true; do
        ffmpeg -re -stream_loop -1 -i "$VIDEO_FILE" \
            -vf "drawtext=text='Camera 0':x=10:y=10:fontsize=24:fontcolor=white:box=1:boxcolor=black@0.5" \
            -c:v libx264 \
            -preset ultrafast \
            -tune zerolatency \
            -s 720x480 \
